  ...
  Route::middleware(['auth', 'verified'])->group(function () {
    Route::get('/dashboard', function () {
      try {
        ...
        $authedUser->avatar = str_contains($authedUser->avatar, 'https')
          ? $authedUser->avatar
          : ($authedUser->avatar ? asset('storage/' . $authedUser->avatar) : null);
        ...
        return Inertia::render('Dashboard', [
          ...
          'evaluations' => in_array($authedUser->role, ['Ne-Waza', 'Fighting'])
            ? Evaluation::where('athlete_id', auth()->id())->get()
            : Evaluation::distinct()->get(['athlete_id'])
              ->pluck('athlete_id')
